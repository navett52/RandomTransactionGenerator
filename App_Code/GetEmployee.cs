...
public class GetEmployee {
    ...
    public int RandomAvailableEmployee() {
        ...
        comm = new SqlCommand(@"SELECT TOP 1 emp.EmplID FROM tEmpl emp JOIN tEmplHistory eh ON emp.EmplID = eh.EmplID
                JOIN tEmplStatus es ON eh.EmplStatusID = es.EmplStatusID WHERE es.CanWork = 1 ORDER BY NEWID()", conn);
